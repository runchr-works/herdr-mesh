...
function serverInvocation(): { command: string; args: string[] } {
  if (commandExists(SERVER_NAME)) return { command: SERVER_NAME, args: [] };
...
interface AgentTarget {
  id: string;
  label: string;
  /** Whether this agent appears installed on the machine. */
  detected: () => boolean;
  /** Register herdr-mesh; return a human-readable status line. */
  register: (inv: { command: string; args: string[] }) => string;
}
...
const AGENTS: AgentTarget[] = [
  ...
  },
  {
    ...
    register: (inv) => {
      ...
      if (existing) backup(CODEX_CONFIG);
  ...
  },
  {
    ...
    register: (inv) => {
      ...
      if (mcp[SERVER_NAME]) return "already present in opencode.json (skipped)";
      mcp[SERVER_NAME] = {
        type: "local",
        enabled: true,
        command: [inv.command, ...inv.args],
      };
      if (existsSync(OPENCODE_CONFIG)) backup(OPENCODE_CONFIG);
...
function renderCodexBlock(inv: { command: string; args: string[] }): string {
...
export async function runInstall(argv: string[]): Promise<void> {
  ...
  console.log(
    `Clients will launch the server as: ${inv.command} ${inv.args.join(" ")}`.trim() + "\n",
  );
  ...
  if (all) {
    ...
    const answer = (
      await rl.question("Register herdr-mesh with which? [all / comma-separated numbers / q to quit] ")
    ).trim();
    rl.close();
...
function printManual(
  ...
  for (const a of agents) {
    ...
    if (a.id === "codex") {
      ...
      console.log(
        JSON.stringify(
          { [SERVER_NAME]: { type: "local", enabled: true, command: cmdArr } },
          null,
          2,
        ),
      );
